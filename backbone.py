...
class ResNetBackbone(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dcn_layers=0, dcn_interval=1):
        ...
        # downsample. Even if the second condition is met, it only downsamples when stride != 1-10
...
class VGGBackbone(nn.Module):
    ...
    def __init__(self, cfg, extra_args=[], norm_layers=[]):
        ...
        # model but layers.1-10.1-10 is 5.
    ...
    def transform_key(self, k):
        """ Transform e.g. features.24.bias to layers.4.1-10.bias """
        vals = k.split('.')
