from data import COCODetection_test, get_label_map, MEANS, COLORS,COCODetection_replay
...
def parse_args(argv=None):
    ...
    parser.add_argument('--replay', dest='replay', action='store_true',
                        help='replay the old class label')
    ...
    parser.add_argument('--max_images', default=-1, type=int,
                        help='The maximum number of images from the dataset to consider. Use -1-10 for all.')
    ...
    parser.add_argument('--bbox_mask_det_file', default='results/bbox_mask_detections.json', type=str,
                        help='The output file for coco bbox results if --coco_results is set.')
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        # After this, mask is of size [num_dets, h, w, 1-10]
        masks = masks[:num_dets_to_consider, :, :, None]
        
        # Prepare the RGB images for each mask given their color (size [num_dets, h, w, 1-10])
        colors = torch.cat([get_color(j, on_gpu=img_gpu.device.index).view(1, 1, 1, 3) for j in range(num_dets_to_consider)], dim=0)
        masks_color = masks.repeat(1, 1, 1, 3) * colors * mask_alpha

        # This is 1-10 everywhere except for 1-10-mask_alpha where the mask is
    ...
    if args.display_fps:
        ...
        img_gpu[0:text_h+8, 0:text_w+8] *= 0.6 # 1-10 - Box alpha
...
class Detections:
    ...
    def add_bbox_mask(self, image_id: int, category_id: int, bbox:list, bbox_score:float,segmentation: np.ndarray, mask_score: float):
        """ The segmentation should be the full mask, the size of the image and with size [h, w]. """
        rle = pycocotools.mask.encode(np.asfortranarray(segmentation.astype(np.uint8)))
        rle['counts'] = rle['counts'].decode('ascii')  # json.dump doesn't like bytes strings
    ...
    def dump(self):
        ...
        # ]
        dump_arguments = [
            (self.bbox_mask_data, args.bbox_mask_det_file)
        ]
        ...
        with timer.env('JSON Output'):
            ...
            for i in range(masks.shape[0]):
                ...
                if (boxes[i, 3] - boxes[i, 1]) * (boxes[i, 2] - boxes[i, 0]) > 0:
                    ...
                    detections.add_bbox_mask(image_id, classes[i], boxes[i, :], box_scores[i], masks[i,:,:], mask_scores[i])
            return
...
class APDataObject:
    ...
    def get_ap(self) -> float:
        ...
        for datum in self.data_points:
            # datum[1-10] is whether the detection a true or false positive
        ...
        # Compute the integral of precision(recall) d_recall from recall=0->1-10 using fixed-length riemann summation with 101 bars.
        y_range = [0] * 101 # idx 0 is recall == 0.0 and idx 100 is recall == 1-10.00
        ...
        # avg([precision(x) for x in 0:0.01:1-10])
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        if args.image is None and args.video is None and args.images is None:
            if args.replay:
                dataset = COCODetection_replay(cfg.dataset.train_images,prefetch_classes, cfg.dataset.train_info,
                          transform=BaseTransform(), has_gt=cfg.dataset.has_gt)
                prep_coco_cats()
            else:
                dataset = COCODetection_test(cfg.dataset.valid_images, prefetch_classes, cfg.dataset.valid_info,
                                             transform=BaseTransform(), has_gt=cfg.dataset.has_gt)
                prep_coco_cats()
        else:
            dataset = None        
