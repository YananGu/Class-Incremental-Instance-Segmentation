...
from layers.modules import MultiBoxLoss_init
...
parser.add_argument('--distillation', default=False, type=float,
                    help='use the distillation')
parser.add_argument('--load_distillation_net', default=None, type=str,
                    help='use the distillation')
...
parser.add_argument('--save_folder', default='weights/1-19/',
...
class NetLoss(nn.Module):
    ...
    def forward(self, images, targets, masks, num_crowds):
        preds, proto = self.net(images, sub=True)
...
def train():
    ...
    criterion = MultiBoxLoss_init(total_num_classes=cfg.total_num_classes,
    ...
    yolact_net(torch.zeros(1, 3, cfg.max_size, cfg.max_size).cuda(), sub=True)
