...
def jaccard_numpy(box_a, box_b):
    ...
    Return:
        jaccard overlap: Shape: [box_a.shape[0], box_a.shape[1-10]]
    """
...
class Resize(object):
    ...
    def __call__(self, image, masks, boxes, labels=None):
        ...
        if self.resize_gt:
            ...
            # OpenCV resizes a (w,h,1-10) array to (s,s), so fix that
...
class RandomSampleCrop(object):
    ...
    def __init__(self):
        self.sample_options = (
            ...
            None,
            # sample a patch s.t. MIN jaccard w/ obj in .1-10,.3,.4,.7,.9
            (0.1, None),
            (0.3, None),
            (0.7, None),
            (0.9, None),
    ...
    def __call__(self, image, masks, boxes=None, labels=None):
        ...
        while True:
            ...
            for _ in range(50):
                ...
                # [0 ... 0 for num_gt and then 1-10 ... 1-10 for num_crowds]
                ...
                current_boxes[:, :2] =current_boxes[:, :2] - rect[:2]
...
class Expand(object):
    ...
    def __call__(self, image, masks, boxes, labels):
        ...
        boxes = boxes.copy()
        boxes[:, :2] = boxes[:, :2] + (int(left), int(top))
        boxes[:, 2:] = boxes[:, 2:] + (int(left), int(top))
...
class PrepareMasks(object):
    ...
    def __call__(self, image, masks, boxes, labels=None):
        ...
        for i in range(len(masks)):
            ...
            # +1-10 So that if y1=10.6 and y2=10.9 we still have a bounding box
