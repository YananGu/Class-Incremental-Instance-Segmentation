...
#
# # This is required for Pytorch 1-10.0.1-10 on Windows to initialize Cuda on some driver versions.
...
#                                        of 1-10) for this layer would produce bounding
...
#         self.total_num_classes = cfg.total_num_classes
...
#             self.conf_layer = nn.Conv2d(out_channels, self.num_priors * self.total_num_classes, **cfg.head_layer_params)
...
#             - class_confs: [batch_size, conv_h*conv_w*num_priors, total_num_classes]
...
#         conf = src.conf_layer(conf_x).permute(0, 2, 3, 1).contiguous().view(x.size(0), -1, self.total_num_classes)
#
...
#         preds['conf'] = preds['conf'][:, :, :len(self.to_learn)]
#         if cfg.use_mask_scoring:
#             preds['score'] = score
#
#         if cfg.use_instance_coeff:
...
#
#
# class PredictionModule_extend(nn.Module):
#     """
#     The (c) prediction module adapted from DSSD:
#     https://arxiv.org/pdf/1701.06659.pdf
#
...
#         priors = self.make_priors(conv_h, conv_w, x.device)
#
#         preds = {'loc_extend': bbox, 'conf_extend': conf, 'mask_extend': mask, 'priors_extend': priors}
#         preds['conf_extend'] = preds['conf_extend'][:, :, :len(self.to_learn)]
#         if cfg.use_mask_scoring:
#             preds['score_extend'] = score
#
#         if cfg.use_instance_coeff:
#             preds['inst_extend'] = inst
#
#         return preds
#
#     def make_priors(self, conv_h, conv_w, device):
...
#         # given in the correct order. Thus, use j=-i-1-10 for the input and output and i for the conv layers.
#         j = len(convouts)
...
#                 # Note: this is an untested alternative to out.append(out[-1-10][:, :, ::2, ::2]). Thanks TorchScript.
...
#         super().__init__()
#         input_channels = 1
#         last_layer = [(cfg.total_num_classes - 1, 1, {})]
...
#                     parent = self.prediction_layers_extend[0]
#
#                 pred = PredictionModule_extend(src_channels[layer_idx], src_channels[layer_idx],
#                                                aspect_ratios=cfg.backbone.pred_aspect_ratios[idx],
#                                                scales=cfg.backbone.pred_scales[idx],
#                                                parent=parent,
#                                                index=idx)
#                 self.prediction_layers_extend.append(pred)
#
#         # Extra parameters for the extra losses
#         if cfg.use_class_existence_loss:
#             # This comes from the smallest layer selected
#             # Also note that cfg.total_num_classes includes background
#             self.class_existence_fc = nn.Linear(src_channels[-1], cfg.total_num_classes - 1)
#
#         if cfg.use_semantic_segmentation_loss:
#             self.semantic_seg_conv = nn.Conv2d(src_channels[0], cfg.total_num_classes - 1, kernel_size=1)
#
#         # For use in evaluation
#         self.detect = Detect(len(self.to_learn), bkg_label=0, top_k=cfg.nms_top_k,
...
#             # Broke in 1-10.3 (see issue #175), WeakScriptModuleProxy was turned into just ScriptModule.
...
#
#         pred_outs_extend = {'loc_extend': [], 'conf_extend': [], 'mask_extend': [], 'priors_extend': []}
#         if sub == False:
#             with timer.env('pred_heads_extend'):
#
#                 if cfg.use_mask_scoring:
#                     pred_outs_extend['score_extend'] = []
#
#                 if cfg.use_instance_coeff:
#                     pred_outs_extend['inst_extend'] = []
#
#                 for idx, pred_layer in zip(self.selected_layers, self.prediction_layers_extend):
#                     pred_x = outs[idx]
#
...
#                     if cfg.share_prediction_module and pred_layer is not self.prediction_layers_extend[0]:
#                         pred_layer.parent = [self.prediction_layers_extend[0]]
#
#                     p = pred_layer(pred_x)
#
#                     for k, v in p.items():
#                         pred_outs_extend[k].append(v)
#
#             for k, v in pred_outs_extend.items():
#                 pred_outs_extend[k] = torch.cat(v, -2)
#
#         if proto_out is not None:
#             # pred_outs['proto'] = proto_out
#             pred_outs_extend['proto'] = proto_out
#         if self.training:
...
#         else:
#             if cfg.use_mask_scoring:
#                 pred_outs_extend['score_extend'] = torch.sigmoid(pred_outs_extend['score_extend'])
#
#             if cfg.use_focal_loss:
#                 if cfg.use_sigmoid_focal_loss:
#                     # Note: even though conf[0] exists, this mode doesn't train it so don't use it
#                     pred_outs_extend['conf_extend'] = torch.sigmoid(pred_outs_extend['conf_extend'])
#                     if cfg.use_mask_scoring:
#                         pred_outs_extend['conf_extend'] *= pred_outs_extend['score_extend']
#                 elif cfg.use_objectness_score:
#                     # See focal_loss_sigmoid in multibox_loss.py for details
#                     objectness = torch.sigmoid(pred_outs_extend['conf_extend'][:, :, 0])
#                     pred_outs_extend['conf_extend'][:, :, 1:] = objectness[:, :, None] * F.softmax(
#                         pred_outs_extend['conf_extend'][:, :, 1:], -1)
#                     pred_outs_extend['conf_extend'][:, :, 0] = 1 - objectness
#                 else:
#                     pred_outs_extend['conf_extend'] = F.softmax(pred_outs_extend['conf_extend'], -1)
#             else:
#
#                 if cfg.use_objectness_score:
#                     objectness = torch.sigmoid(pred_outs_extend['conf_extend'][:, :, 0])
#
#                     pred_outs_extend['conf_extend'][:, :, 1:] = (objectness > 0.10)[..., None] \
#                                                                 * F.softmax(pred_outs_extend['conf_extend'][:, :, 1:],
#                                                                             dim=-1)
#
#                 else:
#                     #  pred_outs['conf'] = F.softmax(pred_outs['conf'][:,:,:len(self.to_learn)], -1)
...
from data.config import cfg, mask_type
from layers import Detect_expert
from layers.interpolate import InterpolateModule
from backbone import construct_backbone
...
# This is required for Pytorch 1-10.0.1-10 on Windows to initialize Cuda on some driver versions.
# See the bug report here: https://github.com/pytorch/pytorch/issues/18108
torch.cuda.current_device()
...
class PredictionModule(nn.Module):
    """
    The (c) prediction module adapted from DSSD:
    https://arxiv.org/pdf/1801.06659.pdf
