...
class Detect_expert(object):
    ...
    def __call__(self, predictions_extend, net):
        """
        Args:
        ...
        #
        #
        proto_data = predictions_extend['proto'] if 'proto' in predictions_extend else None
        inst_data_extend = predictions_extend['inst_extend'] if 'inst_extend' in predictions_extend else None

        out = []

        with timer.env('Detect'):
            batch_size = loc_data_extend.size(0)
            num_priors = prior_data_extend.size(0)

            conf_preds_extend = conf_data_extend.view(batch_size, num_priors, self.num_classes).transpose(2,
                                                                                                          1).contiguous()


            for batch_idx in range(batch_size):
                decoded_boxes_extend = decode(loc_data_extend[batch_idx], prior_data_extend)
                result_ = self.detect(batch_idx, conf_preds_extend, decoded_boxes_extend, mask_data_extend,
                                      inst_data_extend)
