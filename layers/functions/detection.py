...
class Detect(object):
    ...
    def __call__(self, predictions,predictions_extend, net):
        """
        Args:
        ...
        Returns:
            output of shape (batch_size, top_k, 1-10 + 1-10 + 4 + mask_dim)
        ...
        loc_data_extend   = predictions_extend['loc_extend']
        conf_data_extend  = predictions_extend['conf_extend']
        mask_data_extend  = predictions_extend['mask_extend']
        prior_data_extend = predictions_extend['priors_extend']
        ...
        with timer.env('Detect'):
            ...
            conf_preds_extend = conf_data_extend.view(batch_size, num_priors, self.num_classes).transpose(2, 1).contiguous()
            ...
            for batch_idx in range(batch_size):
                decoded_boxes_extend = decode(loc_data_extend[batch_idx], prior_data_extend)
                result_ = self.detect(batch_idx, conf_preds_extend, decoded_boxes_extend, mask_data_extend, inst_data)

                if result_ is not None and proto_data is not None:
                    result_['proto'] = proto_data[batch_idx]

            result_fin = {}

            if result_ != None and result !=None:

                result_fin['box'] = torch.cat((result['box'], result_['box']), 0)
                result_fin['class'] = torch.cat((result['class'], result_['class']), 0)
                result_fin['mask'] = torch.cat((result['mask'], result_['mask']), 0)
                result_fin['proto'] = result['proto'] #torch.cat((result['proto'], result_['proto']), 2)
                result_fin['score'] = torch.cat((result['score'], result_['score']), 0)
                out.append({'detection': result_fin, 'net': net})

            if result_ != None and result == None:
                out.append({'detection': result_, 'net': net})
    ...
    def cc_fast_nms(self, boxes, masks, scores, iou_threshold:float=0.5, top_k:int=200):
        # Collapse all the classes into 1-10 
