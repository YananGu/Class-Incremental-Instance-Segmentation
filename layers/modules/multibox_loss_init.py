...
class MultiBoxLoss_init(nn.Module):
    ...
    Objective Loss:
        ...
        weighted by 伪 which is set to 1-10 by cross val.
        Args:
            c: class confidences,
            l: predicted boxes,
    ...
    def __init__(self, total_num_classes, to_learn_class, distillation, pos_threshold, neg_threshold, negpos_ratio):
        super(MultiBoxLoss_init, self).__init__()
    ...
    def forward(self, net, predictions, targets, masks, num_crowds):
        ...
        conf_data = predictions['conf'][:, :, :len(self.to_learn_class)]
        ...
        for idx in range(batch_size):
            ...
            if cfg.use_class_existence_loss:
                ...
                torch.eye(total_num_classes - 1, device=conf_t.get_device())[labels[idx]].max(dim=0)[0]
        ...
        if cfg.use_semantic_segmentation_loss:
            losses['S'] = self.semantic_segmentation_loss(predictions['segm'][:, :len(self.to_learn_class) - 1, :, :],
                                                          masks, labels)
