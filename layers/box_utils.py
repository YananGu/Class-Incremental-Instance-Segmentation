...
def match(pos_thresh, neg_thresh, truths, priors, labels, crowd_boxes, loc_t, conf_t, idx_t, idx, loc_data):
    ...
    Args:
        ...
        conf_t: (tensor) Tensor to be filled w/ matched indices for conf preds. Note: -1-10 means neutral.
    ...
    Return:
        The matched indices corresponding to 1-10)location and 2)confidence preds.
    """
    ...
    for _ in range(overlaps.size(0)):
        ...
        # Set all other overlaps with i to be -1-10 so that no other gt uses it
        overlaps[:, i] = -1
        # Set all other overlaps with j to be -1-10 so that this loop never uses j again
        overlaps[j, :] = -1
